...
import CropTab from './components/CropTab.jsx';
...
export default function App() {
  ...
  const zoomDelta = (d) => setZoom((z) => Math.min(3, Math.max(0.25, +(z + d).toFixed(2))));
  ...
  return (
    <div onDragOver={e=>{e.preventDefault(); setDragOver(true);}} onDragLeave={()=>setDragOver(false)} onDrop={handleDrop} style={{height:'100%', display:'flex', flexDirection:'column'}}>
      ...
      {activeTab==='caption' ? (
        ...
        <CropTab
          images={images} selected={selected} folder={folder}
          settings={settings} currentSetting={currentSetting} imageData={imageData}
          globalUpscale={globalUpscale} zoom={zoom} batch={batch} dragOver={dragOver}
          onSelect={setSelected} onDelete={onDelete} onRefreshList={refreshList}
          onToggleCrop={toggleCrop} onClearCrop={() => updateSetting(selected, { crop: null })}
          onAspect={handleAspect} onRotate={rotateBy} onResetRotation={() => updateSetting(selected, { rotation: 0 })}
          onToggleUpscale={(v) => updateSetting(selected, { upscaleEnabled: v })}
          onZoomDelta={zoomDelta} onZoomSet={setZoom} onZoomFit={() => setZoom(1)}
          onCropChange={handleCropChange} onGo={go}
        />
      )}
